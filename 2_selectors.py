import selectors # Функция мониторинга над файлами, сокетами, и над всем, что имеет файловый дискриптор 
import socket
...
def send_message(client_socket: str) -> str: # Функция отправки и получения сообщения от пользователя, она будет принимать клиентский сокет 
...
def event_loop(): # Функиция которая определяет вызов нашего кода 
    ...
    """ Метод select работает с любыми файловыми объектами, у которых  есть метод 
